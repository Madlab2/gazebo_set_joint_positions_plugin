...
Setting up ROS2-controlled actuated joints in Gazebo is often tedious when using the traditional [gz_ros2_control](https://github.com/ros-controls/gz_ros2_control/tree/humble) stack. This plugin offers a shortcut by directly applying joint positions from a `sensor_msgs/msg/JointState` topic to the simulated robot, skipping the hassle of an entire ROS2 control stack + gain tuning.

## Try It
```bash
./use_docker.sh run # runs a test launch with gazebo gui and jsp gui (requires xserver).
```

## Use It
1. Clone this repo into a colcon workspace.
2. Install dependencies with: `rosdep install --from-paths src --ignore-src -r -y`
3. Build and source
4. Append the following lines to your URDF/XACRO:
   ```xml
   <robot>

   <!-- the rest of robot xml -->

   <gazebo>
           <plugin
           filename="libgazebo_set_joint_positions_plugin.so"
           name="gazebo_set_joint_positions_plugin::SetJointPositions">
